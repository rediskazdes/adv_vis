...
```{r}
g = ggplot() + geom_tile(data=compare, mapping=aes(x=x_coors[1],y=coor,height=0.85,width=0.75), alpha=0.9) +  # поменяли порядок слоев
...
g = ggplot() + geom_tile(data=compare, mapping=aes(x=x_coors[1],y=coor,height=0.85,width=0.75,fill=region), alpha=0.9) +
               ...
               annotate("text",x=x_coors,y=max(compare$coor+2),label=x_nms,color="white",size=4.3,family = "AvantGarde") # добавляем заголовки
